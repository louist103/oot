#include "z_en_du.h"
#include "objects/object_du/object_du.h"
#include "scenes/overworld/spot18/spot18_scene.h"

#define FLAGS 0x02000009

#define THIS ((EnDu*)thisx)

void EnDu_Init(Actor* thisx, GlobalContext* globalCtx);
void EnDu_Destroy(Actor* thisx, GlobalContext* globalCtx);
void EnDu_Update(Actor* thisx, GlobalContext* globalCtx);
void EnDu_Draw(Actor* thisx, GlobalContext* globalCtx);

void func_809FE3B4(EnDu* self, GlobalContext* globalCtx);
void func_809FE3C0(EnDu* self, GlobalContext* globalCtx);
void func_809FE638(EnDu* self, GlobalContext* globalCtx);
void func_809FE890(EnDu* self, GlobalContext* globalCtx);
void func_809FE4A4(EnDu* self, GlobalContext* globalCtx);
void func_809FE6CC(EnDu* self, GlobalContext* globalCtx);
void func_809FE740(EnDu* self, GlobalContext* globalCtx);
void func_809FE798(EnDu* self, GlobalContext* globalCtx);
void func_809FEC14(EnDu* self, GlobalContext* globalCtx);
void func_809FEC70(EnDu* self, GlobalContext* globalCtx);
void func_809FECE4(EnDu* self, GlobalContext* globalCtx);
void func_809FEB08(EnDu* self, GlobalContext* globalCtx);

const ActorInit En_Du_InitVars = {
    ACTOR_EN_DU,
    ACTORCAT_NPC,
    FLAGS,
    OBJECT_DU,
    sizeof(EnDu),
    (ActorFunc)EnDu_Init,
    (ActorFunc)EnDu_Destroy,
    (ActorFunc)EnDu_Update,
    (ActorFunc)EnDu_Draw,
};

static ColliderCylinderInit sCylinderInit = {
    {
        COLTYPE_NONE,
        AT_NONE,
        AC_NONE,
        OC1_ON | OC1_TYPE_ALL,
        OC2_TYPE_2,
        COLSHAPE_CYLINDER,
    },
    {
        ELEMTYPE_UNK0,
        { 0x00000000, 0x00, 0x00 },
        { 0x00000000, 0x00, 0x00 },
        TOUCH_NONE,
        BUMP_NONE,
        OCELEM_ON,
    },
    { 20, 46, 0, { 0, 0, 0 } },
};

static CollisionCheckInfoInit2 sColChkInfoInit = {
    0, 0, 0, 0, MASS_IMMOVABLE,
};

static struct_80034EC0_Entry sAnimations[] = {
    { &gDaruniaIdleAnim, 1.0f, 0.0f, -1.0f, ANIMMODE_LOOP, 0.0f },
    { &gDaruniaIdleAnim, 1.0f, 0.0f, -1.0f, ANIMMODE_LOOP, -10.0f },
    { &gDaruniaItemGiveAnim, 1.0f, 0.0f, -1.0f, ANIMMODE_LOOP, -10.0f },
    { &gDaruniaItemGiveIdleAnim, 1.0f, 0.0f, -1.0f, ANIMMODE_LOOP, -10.0f },
    { &gDaruniaHitLinkAnim, 1.0f, 0.0f, -1.0f, ANIMMODE_LOOP, -10.0f },
    { &gDaruniaHitBreastAnim, 1.0f, 0.0f, -1.0f, ANIMMODE_LOOP, -10.0f },
    { &gDaruniaStandUpAfterFallingAnim, 1.0f, 0.0f, -1.0f, ANIMMODE_LOOP, -10.0f },
    { &gDaruniaDancingLoop1Anim, 1.0f, 0.0f, -1.0f, ANIMMODE_ONCE, -10.0f },
    { &gDaruniaDancingLoop1Anim, 1.0f, 0.0f, -1.0f, ANIMMODE_ONCE, 0.0f },
    { &gDaruniaDancingLoop2Anim, 1.0f, 0.0f, -1.0f, ANIMMODE_ONCE, 0.0f },
    { &gDaruniaDancingLoop3Anim, 1.0f, 0.0f, -1.0f, ANIMMODE_ONCE, 0.0f },
    { &gDaruniaWrongSongAnim, 1.0f, 0.0f, -1.0f, ANIMMODE_ONCE, 0.0f },
    { &gDaruniaWrongSongEndAnim, 1.0f, 0.0f, -1.0f, ANIMMODE_LOOP, 0.0f },
    { &gDaruniaDancingLoop4Anim, 1.0f, 0.0f, -1.0f, ANIMMODE_ONCE, 0.0f },
    { &gDaruniaDancingEndAnim, 1.0f, 0.0f, -1.0f, ANIMMODE_ONCE, -6.0f },
};

void EnDu_SetupAction(EnDu* self, EnDuActionFunc actionFunc) {
    self->actionFunc = actionFunc;
}

u16 func_809FDC38(GlobalContext* globalCtx, Actor* actor) {
    u16 reaction = Text_GetFaceReaction(globalCtx, 0x21);

    if (reaction != 0) {
        return reaction;
    }
    if (CUR_UPG_VALUE(UPG_STRENGTH) != 0) {
        if (CHECK_QUEST_ITEM(QUEST_GORON_RUBY)) {
            return 0x301E;
        } else {
            return 0x301D;
        }
    }
    if (gSaveContext.infTable[0x11] & 8) {
        return 0x301B;
    } else {
        return 0x301A;
    }
}

s16 func_809FDCDC(GlobalContext* globalCtx, Actor* actor) {
    switch (func_8010BDBC(&globalCtx->msgCtx)) {
        case 0:
        case 1:
            break;
        case 2:
            switch (actor->textId) {
                case 0x301A:
                    gSaveContext.infTable[0x11] |= 8;
                    break;
                case 0x301C:
                case 0x301F:
                    return 2;
                case 0x3020:
                    gSaveContext.eventChkInf[0x2] |= 4;
                    break;
            }
            return 0;
        case 3:
        case 4:
        case 5:
            break;
        case 6:
            if (func_80106BC8(globalCtx)) {
                return 3;
            }
            break;
        case 7:
        case 8:
        case 9:
            break;
    }
    return 1;
}

s32 func_809FDDB4(EnDu* self, GlobalContext* globalCtx) {
    if (globalCtx->sceneNum == SCENE_SPOT18 && LINK_IS_CHILD) {
        return 1;
    } else if (globalCtx->sceneNum == SCENE_HIDAN && !(gSaveContext.infTable[0x11] & 0x400) && LINK_IS_ADULT) {
        return 1;
    }
    return 0;
}

void func_809FDE24(EnDu* self, GlobalContext* globalCtx) {
    Player* player = GET_PLAYER(globalCtx);
    s16 phi_a3 = 0;

    if (self->unk_1F4.unk_00 == 0) {
        phi_a3 = 1;
    }
    if (self->actionFunc == func_809FE890) {
        phi_a3 = 1;
    }
    self->unk_1F4.unk_18 = player->actor.world.pos;
    self->unk_1F4.unk_14 = 10.0f;
    func_80034A14(&self->actor, &self->unk_1F4, 3, phi_a3);
}

void func_809FDE9C(EnDu* self) {
    if (self->blinkTimer > 0) {
        self->blinkTimer--;
    } else {
        self->blinkTimer = 0;
    }
    if (self->blinkTimer < 3) {
        self->eyeTexIndex = self->blinkTimer;
    }

    switch (self->unk_1EC) {
        case 0:
            if (self->blinkTimer == 0) {
                self->blinkTimer = Rand_S16Offset(30, 30);
            }
            break;
        case 1:
            if (self->blinkTimer == 0) {
                self->eyeTexIndex = 2;
            }
            break;
        case 2:
            if (self->blinkTimer == 0) {
                self->eyeTexIndex = 2;
            }
            break;
        case 3:
            if (self->blinkTimer == 0) {
                self->eyeTexIndex = 0;
            }
            break;
    }

    switch (self->unk_1ED) {
        case 1:
            self->mouthTexIndex = 1;
            break;
        case 2:
            self->mouthTexIndex = 2;
            break;
        case 3:
            self->mouthTexIndex = 3;
            break;
        default:
            self->mouthTexIndex = 0;
            break;
    }

    if (self->unk_1EE == 1) {
        self->noseTexIndex = 1;
    } else {
        self->noseTexIndex = 0;
    }
}

void func_809FDFC0(CsCmdActorAction* csAction, Vec3f* dst) {
    dst->x = csAction->startPos.x;
    dst->y = csAction->startPos.y;
    dst->z = csAction->startPos.z;
}

void func_809FE000(CsCmdActorAction* csAction, Vec3f* dst) {
    dst->x = csAction->endPos.x;
    dst->y = csAction->endPos.y;
    dst->z = csAction->endPos.z;
}

void func_809FE040(EnDu* self) {
    s32 animationIndices[] = { 8, 8, 8, 8, 9, 10, 10, 13 };

    if (Animation_OnFrame(&self->skelAnime, self->skelAnime.endFrame)) {
        self->unk_1E6++;
        if (self->unk_1E6 >= 8) {
            self->unk_1E6 = 0;
        }
        func_80034EC0(&self->skelAnime, sAnimations, animationIndices[self->unk_1E6]);
    }
}

void func_809FE104(EnDu* self) {
    s32 animationIndices[] = { 8, 8, 11, 12 };

    if (self->unk_1E6 < 4) {
        if (Animation_OnFrame(&self->skelAnime, self->skelAnime.endFrame)) {
            self->unk_1E6++;
            if (self->unk_1E6 < 4) {
                func_80034EC0(&self->skelAnime, sAnimations, animationIndices[self->unk_1E6]);
            }
        }
    }
}

void EnDu_Init(Actor* thisx, GlobalContext* globalCtx) {
    EnDu* self = THIS;
    s32 pad;

    ActorShape_Init(&self->actor.shape, 0.0f, ActorShadow_DrawCircle, 30.0f);
    SkelAnime_InitFlex(globalCtx, &self->skelAnime, &gDaruniaSkel, NULL, 0, 0, 0);
    Collider_InitCylinder(globalCtx, &self->collider);
    Collider_SetCylinder(globalCtx, &self->collider, &self->actor, &sCylinderInit);
    CollisionCheck_SetInfo2(&self->actor.colChkInfo, DamageTable_Get(0x16), &sColChkInfoInit);
    if (func_809FDDB4(self, globalCtx) == 0) {
        Actor_Kill(&self->actor);
        return;
    }
    func_80034EC0(&self->skelAnime, sAnimations, 0);
    Actor_SetScale(&self->actor, 0.01f);
    self->actor.targetMode = 1;
    self->unk_1F4.unk_00 = 0;

    if (gSaveContext.cutsceneIndex >= 0xFFF0) {
        globalCtx->csCtx.segment = SEGMENTED_TO_VIRTUAL(gGoronCityDarunia01Cs);
        gSaveContext.cutsceneTrigger = 1;
        EnDu_SetupAction(self, func_809FE890);
    } else if (globalCtx->sceneNum == 4) {
        EnDu_SetupAction(self, func_809FE638);
    } else if (!LINK_IS_ADULT) {
        EnDu_SetupAction(self, func_809FE3C0);
    } else {
        EnDu_SetupAction(self, func_809FE3B4);
    }
}

void EnDu_Destroy(Actor* thisx, GlobalContext* globalCtx) {
    EnDu* self = THIS;

    SkelAnime_Free(&self->skelAnime, globalCtx);
    Collider_DestroyCylinder(globalCtx, &self->collider);
}

void func_809FE3B4(EnDu* self, GlobalContext* globalCtx) {
}

void func_809FE3C0(EnDu* self, GlobalContext* globalCtx) {
    Player* player = GET_PLAYER(globalCtx);

    if (player->stateFlags2 & 0x1000000) {
        func_8010BD88(globalCtx, 0x22);
        player->stateFlags2 |= 0x2000000;
        player->unk_6A8 = &self->actor;
        EnDu_SetupAction(self, func_809FE4A4);
        return;
    }
    if (self->unk_1F4.unk_00 == 2) {
        func_8002DF54(globalCtx, &self->actor, 7);
        self->unk_1F4.unk_00 = 0;
    }
    if (self->actor.xzDistToPlayer < 116.0f + self->collider.dim.radius) {
        player->stateFlags2 |= 0x800000;
    }
}

void func_809FE4A4(EnDu* self, GlobalContext* globalCtx) {
    Player* player = GET_PLAYER(globalCtx);

    if (globalCtx->msgCtx.unk_E3EE == 4) {
        globalCtx->msgCtx.unk_E3EE = 0;
        EnDu_SetupAction(self, func_809FE3C0);
    } else if (globalCtx->msgCtx.unk_E3EE >= 6) {
        globalCtx->csCtx.segment = SEGMENTED_TO_VIRTUAL(gGoronCityDaruniaWrongCs);
        gSaveContext.cutsceneTrigger = 1;
        self->unk_1E8 = 1;
        EnDu_SetupAction(self, func_809FE890);
        globalCtx->msgCtx.unk_E3EE = 4;
    } else if (globalCtx->msgCtx.unk_E3EE == 3) {
        Audio_PlaySoundGeneral(NA_SE_SY_CORRECT_CHIME, &D_801333D4, 4, &D_801333E0, &D_801333E0, &D_801333E8);
        globalCtx->csCtx.segment = SEGMENTED_TO_VIRTUAL(gGoronCityDaruniaCorrectCs);
        gSaveContext.cutsceneTrigger = 1;
        self->unk_1E8 = 0;
        EnDu_SetupAction(self, func_809FE890);
        globalCtx->msgCtx.unk_E3EE = 4;
    } else {
        player->stateFlags2 |= 0x800000;
    }
}

void func_809FE638(EnDu* self, GlobalContext* globalCtx) {
    Player* player = GET_PLAYER(globalCtx);

    if (!(player->stateFlags1 & 0x20000000)) {
        OnePointCutscene_Init(globalCtx, 3330, -99, &self->actor, MAIN_CAM);
        player->actor.shape.rot.y = player->actor.world.rot.y = self->actor.world.rot.y + 0x7FFF;
        func_800F5C64(0x51);
        EnDu_SetupAction(self, func_809FE6CC);
        self->unk_1E2 = 0x32;
    }
}

void func_809FE6CC(EnDu* self, GlobalContext* globalCtx) {
    s16 phi_v1;

    if (self->unk_1E2 == 0) {
        phi_v1 = 0;
    } else {
        self->unk_1E2--;
        phi_v1 = self->unk_1E2;
    }
    if (phi_v1 == 0) {
        self->actor.textId = 0x3039;
        func_8010B680(globalCtx, self->actor.textId, NULL);
        self->unk_1F4.unk_00 = 1;
        EnDu_SetupAction(self, func_809FE740);
    }
}

void func_809FE740(EnDu* self, GlobalContext* globalCtx) {
    if (self->unk_1F4.unk_00 == 0) {
        func_8005B1A4(GET_ACTIVE_CAM(globalCtx));
        self->unk_1E2 = 0x5A;
        EnDu_SetupAction(self, func_809FE798);
    }
}

void func_809FE798(EnDu* self, GlobalContext* globalCtx) {
    s32 phi_v0;

    if (self->unk_1E2 == 0) {
        phi_v0 = 0;
    } else {
        self->unk_1E2--;
        phi_v0 = self->unk_1E2;
    }
    if (phi_v0 != 0) {
        switch (self->unk_1E2) {
            case 0x50:
                Audio_PlayActorSound2(&self->actor, NA_SE_EV_CHAIN_KEY_UNLOCK_B);
                break;
            case 0x3C:
                Audio_PlayActorSound2(&self->actor, NA_SE_EV_SLIDE_DOOR_OPEN);
                break;
            case 0xF:
                Audio_PlayActorSound2(&self->actor, NA_SE_EV_SLIDE_DOOR_CLOSE);
                break;
            case 5:
                Audio_PlayActorSound2(&self->actor, NA_SE_EV_STONE_BOUND);
                break;
        }
        if (self->unk_1E2 >= 0x3D) {
            self->actor.world.pos.x -= 10.0f;
        }
    } else {
        Actor_Kill(&self->actor);
        gSaveContext.infTable[0x11] |= 0x400;
    }
}

void func_809FE890(EnDu* self, GlobalContext* globalCtx) {
    f32 frame;
    Vec3f startPos;
    Vec3f endPos;
    Vec3f velocity = { 0.0f, 0.0f, 0.0f };
    CsCmdActorAction* csAction;

    if (globalCtx->csCtx.state == CS_STATE_IDLE) {
        func_8002DF54(globalCtx, &self->actor, 1);
        EnDu_SetupAction(self, func_809FEB08);
        return;
    }
    csAction = globalCtx->csCtx.npcActions[2];

    if (csAction != NULL) {
        func_809FDFC0(csAction, &startPos);
        func_809FE000(csAction, &endPos);
        if (self->unk_1EA == 0) {
            func_809FDFC0(csAction, &startPos);
            self->actor.world.pos = startPos;
        }
        if (self->unk_1EA != csAction->action) {
            if (csAction->action == 1) {
                func_80034EC0(&self->skelAnime, sAnimations, 1);
            }
            if (csAction->action == 7 || csAction->action == 8) {
                self->unk_1E6 = 0;
                func_80034EC0(&self->skelAnime, sAnimations, 7);
            }
            self->unk_1EA = csAction->action;
            if (self->unk_1EA == 7) {
                self->blinkTimer = 11;
                self->unk_1EC = 2;
                self->unk_1ED = 2;
                self->unk_1EE = 1;
            }
            if (self->unk_1EA == 8) {
                self->blinkTimer = 11;
                self->unk_1EC = 3;
                self->unk_1ED = 3;
                self->unk_1EE = 0;
            }
        }
        if (self->unk_1EA == 7) {
            func_809FE040(self);
        }
        if (self->unk_1EA == 8) {
            func_809FE104(self);
        }
        self->actor.shape.rot.x = csAction->urot.x;
        self->actor.shape.rot.y = csAction->urot.y;
        self->actor.shape.rot.z = csAction->urot.z;
        self->actor.velocity = velocity;

        if (globalCtx->csCtx.frames < csAction->endFrame) {
            frame = csAction->endFrame - csAction->startFrame;

            self->actor.velocity.x = (endPos.x - startPos.x) / frame;
            self->actor.velocity.y = (endPos.y - startPos.y) / frame;
            self->actor.velocity.y += self->actor.gravity;
            if (self->actor.velocity.y < self->actor.minVelocityY) {
                self->actor.velocity.y = self->actor.minVelocityY;
            }
            self->actor.velocity.z = (endPos.z - startPos.z) / frame;
        }
    }
}

void func_809FEB08(EnDu* self, GlobalContext* globalCtx) {
    self->blinkTimer = 11;
    self->unk_1EC = 0;
    self->unk_1ED = 0;
    self->unk_1EE = 0;

    if (self->unk_1E8 == 1) {
        func_8002DF54(globalCtx, &self->actor, 7);
        func_80034EC0(&self->skelAnime, sAnimations, 1);
        EnDu_SetupAction(self, func_809FE3C0);
        return;
    }
    if (CUR_UPG_VALUE(UPG_STRENGTH) <= 0) {
        self->actor.textId = 0x301C;
        EnDu_SetupAction(self, func_809FEC14);
    } else {
        self->actor.textId = 0x301F;
        EnDu_SetupAction(self, func_809FE3C0);
    }
    func_8010B680(globalCtx, self->actor.textId, NULL);
    func_80034EC0(&self->skelAnime, sAnimations, 14);
    self->unk_1F4.unk_00 = 1;
}

void func_809FEC14(EnDu* self, GlobalContext* globalCtx) {
    if (self->unk_1F4.unk_00 == 2) {
        func_8002DF54(globalCtx, &self->actor, 7);
        EnDu_SetupAction(self, func_809FEC70);
        func_809FEC70(self, globalCtx);
    }
}

void func_809FEC70(EnDu* self, GlobalContext* globalCtx) {
    if (Actor_HasParent(&self->actor, globalCtx)) {
        self->actor.parent = NULL;
        EnDu_SetupAction(self, func_809FECE4);
    } else {
        f32 xzRange = self->actor.xzDistToPlayer + 1.0f;
        func_8002F434(&self->actor, globalCtx, GI_BRACELET, xzRange, fabsf(self->actor.yDistToPlayer) + 1.0f);
    }
}

void func_809FECE4(EnDu* self, GlobalContext* globalCtx) {
    if (self->unk_1F4.unk_00 == 3) {
        self->unk_1F4.unk_00 = 0;
        EnDu_SetupAction(self, func_809FE3C0);
    }
}

void EnDu_Update(Actor* thisx, GlobalContext* globalCtx) {
    EnDu* self = THIS;
    s32 pad;

    Collider_UpdateCylinder(&self->actor, &self->collider);
    CollisionCheck_SetOC(globalCtx, &globalCtx->colChkCtx, &self->collider.base);

    if (self->skelAnime.animation == &gDaruniaDancingEndAnim &&
        Animation_OnFrame(&self->skelAnime, self->skelAnime.endFrame)) {
        func_80034EC0(&self->skelAnime, sAnimations, 1);
    }

    SkelAnime_Update(&self->skelAnime);
    func_809FDE9C(self);
    func_809FDE24(self, globalCtx);

    if (self->actionFunc == func_809FE890) {
        self->actor.world.pos.x += self->actor.velocity.x;
        self->actor.world.pos.y += self->actor.velocity.y;
        self->actor.world.pos.z += self->actor.velocity.z;
    } else {
        func_8002D7EC(&self->actor);
    }

    Actor_UpdateBgCheckInfo(globalCtx, &self->actor, 0.0f, 0.0f, 0.0f, 4);

    if (self->actionFunc != func_809FE4A4) {
        func_800343CC(globalCtx, &self->actor, &self->unk_1F4.unk_00, self->collider.dim.radius + 116.0f, func_809FDC38,
                      func_809FDCDC);
    }
    self->actionFunc(self, globalCtx);
}

s32 EnDu_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx,
                          Gfx** gfx) {
    EnDu* self = THIS;
    Vec3s sp1C;

    if (limbIndex == 16) {
        Matrix_Translate(2400.0f, 0.0f, 0.0f, MTXMODE_APPLY);
        sp1C = self->unk_1F4.unk_08;
        Matrix_RotateX(BINANG_TO_RAD(sp1C.y), MTXMODE_APPLY);
        Matrix_RotateZ(BINANG_TO_RAD(sp1C.x), MTXMODE_APPLY);
        Matrix_Translate(-2400.0f, 0.0f, 0.0f, MTXMODE_APPLY);
    }
    if (limbIndex == 8) {
        sp1C = self->unk_1F4.unk_0E;
        Matrix_RotateY(BINANG_TO_RAD(sp1C.y), MTXMODE_APPLY);
        Matrix_RotateX(BINANG_TO_RAD(sp1C.x), MTXMODE_APPLY);
    }
    return 0;
}

void EnDu_PostLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx, Gfx** gfx) {
    EnDu* self = THIS;
    Vec3f D_809FF40C = { 0.0f, -1000.0f, 0.0f };

    if (limbIndex == 16) {
        Matrix_MultVec3f(&D_809FF40C, &self->actor.focus.pos);
    }
}

void EnDu_Draw(Actor* thisx, GlobalContext* globalCtx) {
    static void* eyeTextures[] = {
        gDaruniaEyeOpenTex,
        gDaruniaEyeOpeningTex,
        gDaruniaEyeShutTex,
        gDaruniaEyeClosingTex,
    };
    static void* mouthTextures[] = {
        gDaruniaMouthSeriousTex,
        gDaruniaMouthGrinningTex,
        gDaruniaMouthOpenTex,
        gDaruniaMouthHappyTex,
    };
    static void* noseTextures[] = {
        gDaruniaNoseSeriousTex,
        gDaruniaNoseHappyTex,
    };
    EnDu* self = THIS;

    OPEN_DISPS(globalCtx->state.gfxCtx, "../z_en_du.c", 1470);

    gSPSegment(POLY_OPA_DISP++, 0x08, SEGMENTED_TO_VIRTUAL(eyeTextures[self->eyeTexIndex]));
    gSPSegment(POLY_OPA_DISP++, 0x09, SEGMENTED_TO_VIRTUAL(mouthTextures[self->mouthTexIndex]));
    gSPSegment(POLY_OPA_DISP++, 0x0A, SEGMENTED_TO_VIRTUAL(noseTextures[self->noseTexIndex]));

    func_80034BA0(globalCtx, &self->skelAnime, EnDu_OverrideLimbDraw, EnDu_PostLimbDraw, &self->actor, 255);

    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_en_du.c", 1487);
}
