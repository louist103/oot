...
void EnTp_Init(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    if (this->actor.params <= TAILPASARAN_HEAD) {
        ...
        this->actor.flags |= ACTOR_FLAG_TARGETABLE | ACTOR_FLAG_2 | ACTOR_FLAG_NO_UPDATE_CULLING;
        ...
        for (i = 0; i <= 6; i++) {
            ...
            if (next != NULL) {
                ...
                if (i == 2) {
                    next->actor.flags |= ACTOR_FLAG_TARGETABLE | ACTOR_FLAG_2 | ACTOR_FLAG_NO_UPDATE_CULLING;
                    next->unk_150 = 1; // Why?
                }
