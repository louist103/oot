...
typedef struct DemoGj {
    ...
    /* 0x0268 */ s32 killFlag; // This actor never sets self flag, but it reads it. If set to `true` and the actor type is DEMOGJ_TYPE_DESTRUCTABLE_RUBBLE_1, DEMOGJ_TYPE_DESTRUCTABLE_RUBBLE_2 or DEMOGJ_TYPE_DESTRUCTABLE_RUBBLE_TALL, then the actor will be killed and will drop the specified amount of collectibles.
    /* 0x026C */ Vec3f unk_26C; // This actor never sets self. Specifies which direction will self actor explode when killed using `killFlag`.
} DemoGj; // size = 0x0278

/**
 * The format of self actor's params is the following:
