...
void func_80B92C5C(ObjElevator* self);
void func_80B92C80(ObjElevator* self, GlobalContext* globalCtx);
void func_80B92D20(ObjElevator* self);
void func_80B92D44(ObjElevator* self, GlobalContext* globalCtx);
...
void ObjElevator_SetupAction(ObjElevator* self, ObjElevatorActionFunc actionFunc) {
    self->actionFunc = actionFunc;
}

void func_80B92B08(ObjElevator* self, GlobalContext* globalCtx, CollisionHeader* collision, s32 flag) {
    s16 pad1;
...
void ObjElevator_Init(Actor* thisx, GlobalContext* globalCtx) {
    ObjElevator* self = THIS;
    f32 temp_f0;

    func_80B92B08(self, globalCtx, &object_d_elevator_Col_000360, DPM_PLAYER);
    Actor_SetScale(thisx, sScales[thisx->params & 1]);
    Actor_ProcessInitChain(thisx, sInitChain);
    temp_f0 = (thisx->params >> 8) & 0xF;
    self->unk_16C = temp_f0 + temp_f0;
    func_80B92C5C(self);
...
void func_80B92C5C(ObjElevator* self) {
    ObjElevator_SetupAction(self, func_80B92C80);
}
...
void func_80B92C80(ObjElevator* self, GlobalContext* globalCtx) {
    ...
    if ((self->dyna.unk_160 & 2) && !(self->unk_170 & 2)) {
        sub = thisx->world.pos.y - thisx->home.pos.y;
        if (fabsf(sub) < 0.1f) {
            self->unk_168 = thisx->home.pos.y + ((thisx->params >> 0xC) & 0xF) * 80.0f;
        } else {
            self->unk_168 = thisx->home.pos.y;
        }
        func_80B92D20(self);
    }
}
...
void func_80B92D20(ObjElevator* self) {
    ObjElevator_SetupAction(self, func_80B92D44);
}

void func_80B92D44(ObjElevator* self, GlobalContext* globalCtx) {
    Actor* thisx = &self->dyna.actor;

    if (fabsf(Math_SmoothStepToF(&thisx->world.pos.y, self->unk_168, 1.0f, self->unk_16C, 0.0f)) < 0.001f) {
