...
void func_80B3A15C(EnWonderTalk2* this, GlobalContext* globalCtx) {
    ...
    if ((this->switchFlag >= 0) && Flags_GetSwitch(globalCtx, this->switchFlag)) {
        ...
        if ((this->switchFlag >= 0) && (this->talkMode != 2)) {
            ...
            PRINTF(VT_FGCOL(PURPLE) "☆☆☆☆☆ セーブしたよ！おもいっきり！ %x\n" VT_RST, this->switchFlag);
        }
        ...
        if (!((this->actor.xzDistToPlayer > 40.0f + this->triggerRange) ||
            ...
            if (this->unk_158 >= 2) {
                ...
                PRINTF(VT_FGCOL(PURPLE) "☆☆☆☆☆ 指定メッセージ種類     %x\n" VT_RST, this->baseMsgId);
                // Actual message type
                ...
                switch (this->talkMode) {
                    case 0:
                        // Normal
                        PRINTF(VT_FGCOL(PURPLE) " ☆☆ 通常 ☆☆ \n" VT_RST);
                        break;
                    case 2:
                        // Check only
                        PRINTF(VT_FGCOL(PURPLE) " ☆☆ チェックのみ ☆☆ \n" VT_RST);
                        break;
                    case 3:
                        // Lock only
                        PRINTF(VT_FGCOL(PURPLE) " ☆☆ ロックのみ ☆☆ \n" VT_RST);
                        break;
                }
            }
    ...
    switch (func_8010BDBC(&globalCtx->msgCtx)) {
        ...
        case 0:
            if ((this->switchFlag >= 0) && (this->talkMode != 4)) {
                ...
                PRINTF(VT_FGCOL(PURPLE) "☆☆☆☆☆ (強制)セーブしたよ！おもいっきり！ %x\n" VT_RST, this->switchFlag);
            }
