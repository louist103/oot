...
void DoorWarp1_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (this->actor.params != 2 && this->actor.params != -2 && this->actor.params != 4 && this->actor.params != 6) {
        Lights_PointNoGlowSetInfo(&this->unk_1C8, this->actor.posRot.pos.x, this->actor.posRot.pos.y,
                                  this->actor.posRot.pos.z, 0, 0, 0, 0);
        this->unk_1C4 = LightContext_InsertLight(globalCtx2, &globalCtx2->lightCtx, &this->unk_1C8);
        Lights_PointNoGlowSetInfo(&this->unk_1DC, this->actor.posRot.pos.x, this->actor.posRot.pos.y,
                                  this->actor.posRot.pos.z, 0, 0, 0, 0);
...
void func_8099898C(DoorWarp1* this, GlobalContext* globalCtx) {
    ...
    switch (this->actor.params - 1) {
        ...
        default:
            Lights_PointNoGlowSetInfo(&this->unk_1C8, this->actor.posRot.pos.x, this->actor.posRot.pos.y,
                                      this->actor.posRot.pos.z, 200, 255, 255, 255);
            Lights_PointNoGlowSetInfo(&this->unk_1DC, this->actor.posRot.pos.x, this->actor.posRot.pos.y,
                                      this->actor.posRot.pos.z, 200, 255, 255, 255);
        case 0:
        case 1:
...
void func_80998C90(DoorWarp1* this, GlobalContext* globalCtx) {
    ...
    this->unk_19C = 0.0f;
    Lights_PointNoGlowSetInfo(&this->unk_1C8, this->actor.posRot.pos.x, this->actor.posRot.pos.y,
                              this->actor.posRot.pos.z, 0xC8, 0xFF, 0xFF, 0xFF);
    Lights_PointNoGlowSetInfo(&this->unk_1DC, this->actor.posRot.pos.x, this->actor.posRot.pos.y,
                              this->actor.posRot.pos.z, 0xC8, 0xFF, 0xFF, 0xFF);
...
void func_80998E5C(DoorWarp1* this, GlobalContext* globalCtx) {
    ...
    SkelAnime_Init(globalCtx, &this->skelAnime, &D_06002CA8, &D_06001374, 0, 0, 0);
    SkelAnime_ChangeAnimImpl(&this->skelAnime, &D_06001374, 0, SkelAnime_GetFrameCount(&D_06001374.genericHeader),
...
void func_80998FF4(DoorWarp1* this, GlobalContext* globalCtx) {
    ...
    SkelAnime_ChangeAnimImpl(
        &this->skelAnime, &D_06001374, 0,
        // this arg is saved and loaded from the stack when the second GetFrameCount is called, offset off by 4
        SkelAnime_GetFrameCount(&D_06001374.genericHeader), SkelAnime_GetFrameCount(&D_06001374.genericHeader), 2, 0.0f,
        1);
    this->skelAnime.animCurrentFrame = SkelAnime_GetFrameCount(&D_06001374.genericHeader);
    this->unk_1AE = 0x78;
    this->unk_1B0 = 0xE6;
...
void func_80999A68(DoorWarp1* this, GlobalContext* globalCtx) {
    ...
    Lights_PointNoGlowSetInfo(&this->unk_1C8, (s16)player->actor.posRot.pos.x + 10.0f,
                              (s16)player->actor.posRot.pos.y + 10.0f, (s16)player->actor.posRot.pos.z + 10.0f, 0xEB,
                              0xFF, 0xFF, 0xFF);
    Lights_PointNoGlowSetInfo(&this->unk_1DC, (s16)player->actor.posRot.pos.x - 10.0f,
                              (s16)player->actor.posRot.pos.y - 10.0f, (s16)player->actor.posRot.pos.z - 10.0f, 0xEB,
                              0xFF, 0xFF, 0xFF);
...
void func_8099A508(DoorWarp1* this, GlobalContext* globalCtx) {
    ...
    SkelAnime_ChangeAnimImpl(&this->skelAnime, &D_06001374, 1.0f, SkelAnime_GetFrameCount(&D_06001374.genericHeader),
...
void func_8099B140(DoorWarp1* this, GlobalContext* globalCtx) {
    ...
    OPEN_DISPS(globalCtx->state.gfxCtx, " ", 2078);
    ...
    CLOSE_DISPS(globalCtx->state.gfxCtx, " ", 2098);
...
void func_8099B33C(DoorWarp1* this, GlobalContext* globalCtx) {
    ...
    OPEN_DISPS(globalCtx->state.gfxCtx, " ", 2122);
    ...
    CLOSE_DISPS(globalCtx->state.gfxCtx, " ", 2152);
    SkelAnime_FrameUpdateMatrix(&this->skelAnime);
}
...
void func_8099B5EC(DoorWarp1* this, GlobalContext* globalCtx) {
    ...
    OPEN_DISPS(globalCtx->state.gfxCtx, " ", 2173);
    ...
    gSPSegment(POLY_XLU_DISP++, 0x0A, Matrix_NewMtx(globalCtx->state.gfxCtx, " ", 2247));
    Matrix_Push();
    ...
    gSPSegment(POLY_XLU_DISP++, 0x09, Matrix_NewMtx(globalCtx->state.gfxCtx, " ", 2267));
    ...
    if (this->unk_1A0 > 0.0f) {
        ...
        Matrix_Scale(temp_f12, 1.0f, temp_f12, MTXMODE_APPLY);
        gSPSegment(POLY_XLU_DISP++, 0x09, Matrix_NewMtx(globalCtx->state.gfxCtx, " ", 2336));
        gSPDisplayList(POLY_XLU_DISP++, D_060001A0);
    }

    CLOSE_DISPS(globalCtx->state.gfxCtx, " ", 2340);
}
