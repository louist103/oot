...
u16 func_80AA19A0(GlobalContext* globalCtx, Actor* self);
s16 func_80AA1A38(GlobalContext* globalCtx, Actor* self);

void func_80AA1AE4(EnMa2* self, GlobalContext* globalCtx);
s32 func_80AA1C68(EnMa2* self);
void func_80AA1CC0(EnMa2* self);
void func_80AA1DB4(EnMa2* self, GlobalContext* globalCtx);
void func_80AA2018(EnMa2* self, GlobalContext* globalCtx);
void func_80AA204C(EnMa2* self, GlobalContext* globalCtx);
void func_80AA20E4(EnMa2* self, GlobalContext* globalCtx);
void func_80AA21C8(EnMa2* self, GlobalContext* globalCtx);
...
void func_80AA1CC0(EnMa2* self) {
    if ((!func_80AA1C68(self)) && (DECR(self->unk_20C) == 0)) {
        self->unk_20E += 1;
        if (self->unk_20E >= 3) {
...
void func_80AA1D44(EnMa2* self, s32 idx) {
    f32 frameCount = Animation_GetLastFrame(D_80AA2858[idx].animation);

    Animation_Change(&self->skelAnime, D_80AA2858[idx].animation, 1.0f, 0.0f, frameCount, D_80AA2858[idx].mode,
...
void EnMa2_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    switch (func_80AA1B58(self, globalCtx)) {
        case 1:
            func_80AA1D44(self, 2);
            self->actionFunc = func_80AA2018;
            break;
        case 2:
            func_80AA1D44(self, 3);
            self->actionFunc = func_80AA204C;
            break;
        case 3:
            if (gSaveContext.infTable[8] & 0x2000) {
                func_80AA1D44(self, 0);
            } else {
                func_80AA1D44(self, 3);
            }
            self->actionFunc = func_80AA2018;
...
void EnMa2_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    func_80AA1DB4(self, globalCtx);
    func_80AA1AE4(self, globalCtx);
    if (self->actionFunc != func_80AA20E4) {
        func_800343CC(globalCtx, &self->actor, &self->unk_1E0.unk_00, (f32)self->collider.dim.radius + 30.0f,
...
void EnMa2_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    gSPSegment(POLY_OPA_DISP++, 0x09, SEGMENTED_TO_VIRTUAL(D_80AA28B4[self->unk_210]));
    gSPSegment(POLY_OPA_DISP++, 0x08, SEGMENTED_TO_VIRTUAL(D_80AA28C0[self->unk_20E]));

    SkelAnime_DrawFlexOpa(globalCtx, self->skelAnime.skeleton, self->skelAnime.jointTable, self->skelAnime.dListCount,
                          EnMa2_OverrideLimbDraw, EnMa2_PostLimbDraw, self);
