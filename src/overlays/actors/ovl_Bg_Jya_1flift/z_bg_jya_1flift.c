...
void BgJya1flift_Move(BgJya1flift* this, GlobalContext* globalCtx) {
    ...
    if(CHECK_BTN_ANY(input[0].press.button, BTN_CUP)){
        this->dyna.actor.posRot.pos = player->actor.posRot.pos;
    }

    if (func_8004356C(&this->dyna)){
        if(CHECK_BTN_ANY(input[0].press.button, BTN_A)){
        ...
        if (CHECK_BTN_ANY(input[0].press.button, BTN_CLEFT)) {
        ...
        if (CHECK_BTN_ANY(input[0].press.button, BTN_CRIGHT)) {
            this->dyna.actor.posRot.pos.y += 5.0f;
