...
void EnDoor_SetupType(EnDoor* self, GlobalContext* globalCtx);
void EnDoor_Idle(EnDoor* self, GlobalContext* globalCtx);
void EnDoor_WaitForCheck(EnDoor* self, GlobalContext* globalCtx);
void EnDoor_Check(EnDoor* self, GlobalContext* globalCtx);
void EnDoor_AjarWait(EnDoor* self, GlobalContext* globalCtx);
void EnDoor_AjarOpen(EnDoor* self, GlobalContext* globalCtx);
void EnDoor_AjarClose(EnDoor* self, GlobalContext* globalCtx);
void EnDoor_Open(EnDoor* self, GlobalContext* globalCtx);
...
static EnDoorInfo sDoorInfo[] = {
    ...
    // KEEP objects should remain last and in self order
...
void EnDoor_Init(Actor* thisx, GlobalContext* globalCtx2) {
    ...
    SkelAnime_Init(globalCtx, &self->skelAnime, &gDoorSkel, &gDoor3Anim, self->jointTable, self->morphTable, 5);
    ...
    self->dListIndex = objectInfo->dListIndex;
    ...
    self->dListIndex = objectInfo->dListIndex;
    if (self->actor.objBankIndex == self->requiredObjBankIndex) {
        EnDoor_SetupType(self, globalCtx);
    } else {
        self->actionFunc = EnDoor_SetupType;
    }

    // Double doors
    if (self->actor.params & 0x40) {
        EnDoor* other;

        xOffset = Math_CosS(self->actor.shape.rot.y) * 30.0f;
        zOffset = Math_SinS(self->actor.shape.rot.y) * 30.0f;
        other = (EnDoor*)Actor_SpawnAsChild(&globalCtx->actorCtx, &self->actor, globalCtx, ACTOR_EN_DOOR,
                                            self->actor.world.pos.x + xOffset, self->actor.world.pos.y,
                                            self->actor.world.pos.z - zOffset, 0, self->actor.shape.rot.y + 0x8000, 0,
                                            self->actor.params & ~0x40);
        if (other != NULL) {
            other->unk_192 = 1;
        }
        self->actor.world.pos.x -= xOffset;
        self->actor.world.pos.z += zOffset;
    }
...
void EnDoor_Destroy(Actor* thisx, GlobalContext* globalCtx) {
    ...
    transitionEntry = &globalCtx->transiActorCtx.list[(u16)self->actor.params >> 0xA];
...
void EnDoor_SetupType(EnDoor* self, GlobalContext* globalCtx) {
    ...
    if (Object_IsLoaded(&globalCtx->objectCtx, self->requiredObjBankIndex)) {
        doorType = self->actor.params >> 7 & 7;
        self->actor.flags &= ~0x10;
        self->actor.objBankIndex = self->requiredObjBankIndex;
        self->actionFunc = EnDoor_Idle;
        if (doorType == DOOR_EVENING) {
            doorType =
                (gSaveContext.dayTime > 0xC000 && gSaveContext.dayTime < 0xE000) ? DOOR_SCENEEXIT : DOOR_CHECKABLE;
        }
        self->actor.world.rot.y = 0x0000;
        if (doorType == DOOR_LOCKED) {
            if (!Flags_GetSwitch(globalCtx, self->actor.params & 0x3F)) {
                self->lockTimer = 10;
            }
        } else if (doorType == DOOR_AJAR) {
            if (Actor_WorldDistXZToActor(&self->actor, &GET_PLAYER(globalCtx)->actor) > DOOR_AJAR_SLAM_RANGE) {
                self->actionFunc = EnDoor_AjarWait;
                self->actor.world.rot.y = -0x1800;
            }
        } else if (doorType == DOOR_CHECKABLE) {
            self->actor.textId = (self->actor.params & 0x3F) + 0x0200;
            if (self->actor.textId == 0x0229 && !(gSaveContext.eventChkInf[1] & 0x10)) {
                // Talon's house door. If Talon has not been woken up at Hyrule Castle
                // self door should be openable at any time of day. Note that there is no
                ...
                self->actionFunc = EnDoor_WaitForCheck;
                self->actor.flags |= 0x8000009;
            }
        }
        // Replace the door type it was loaded with by the new type
        self->actor.params = (self->actor.params & ~0x380) | (doorType << 7);
    }
}
...
void EnDoor_Idle(EnDoor* self, GlobalContext* globalCtx) {
    ...
    if (self->playerIsOpening != 0) {
        self->actionFunc = EnDoor_Open;
        Animation_PlayOnceSetSpeed(&self->skelAnime, D_809FCECC[self->animStyle],
                                   (player->stateFlags1 & 0x8000000) ? 0.75f : 1.5f);
        if (self->lockTimer != 0) {
            gSaveContext.inventory.dungeonKeys[gSaveContext.mapIndex]--;
            Flags_SetSwitch(globalCtx, self->actor.params & 0x3F);
            Audio_PlayActorSound2(&self->actor, NA_SE_EV_CHAIN_KEY_UNLOCK);
        }
    } else if (!Player_InCsMode(globalCtx)) {
        if (fabsf(playerPosRelToDoor.y) < 20.0f && fabsf(playerPosRelToDoor.x) < 20.0f &&
            fabsf(playerPosRelToDoor.z) < 50.0f) {
            phi_v0 = player->actor.shape.rot.y - self->actor.shape.rot.y;
        ...
        } else if (doorType == DOOR_AJAR && self->actor.xzDistToPlayer > DOOR_AJAR_OPEN_RANGE) {
            self->actionFunc = EnDoor_AjarOpen;
        }
    }
}
...
void EnDoor_WaitForCheck(EnDoor* self, GlobalContext* globalCtx) {
    if (func_8002F194(&self->actor, globalCtx)) {
        self->actionFunc = EnDoor_Check;
    } else {
        func_8002F2CC(&self->actor, globalCtx, DOOR_CHECK_RANGE);
    }
}
...
void EnDoor_AjarWait(EnDoor* self, GlobalContext* globalCtx) {
    if (self->actor.xzDistToPlayer < DOOR_AJAR_SLAM_RANGE) {
        self->actionFunc = EnDoor_AjarClose;
    }
}
...
void EnDoor_AjarOpen(EnDoor* self, GlobalContext* globalCtx) {
    if (self->actor.xzDistToPlayer < DOOR_AJAR_SLAM_RANGE) {
        self->actionFunc = EnDoor_AjarClose;
    } else if (Math_ScaledStepToS(&self->actor.world.rot.y, -0x1800, 0x100)) {
        self->actionFunc = EnDoor_AjarWait;
    }
}

void EnDoor_AjarClose(EnDoor* self, GlobalContext* globalCtx) {
    if (Math_ScaledStepToS(&self->actor.world.rot.y, 0, 0x700)) {
        self->actionFunc = EnDoor_Idle;
    }
}
...
void EnDoor_Open(EnDoor* self, GlobalContext* globalCtx) {
    ...
    if (DECR(self->lockTimer) == 0) {
        if (SkelAnime_Update(&self->skelAnime)) {
            self->actionFunc = EnDoor_Idle;
            self->playerIsOpening = 0;
        } else if (Animation_OnFrame(&self->skelAnime, sDoorAnimOpenFrames[self->animStyle])) {
            Audio_PlayActorSound2(&self->actor,
                                  (globalCtx->sceneNum == SCENE_HAKADAN || globalCtx->sceneNum == SCENE_HAKADANCH ||
                                   globalCtx->sceneNum == SCENE_HIDAN)
                                      ? NA_SE_EV_IRON_DOOR_OPEN
                                      : NA_SE_OC_DOOR_OPEN);
            if (self->skelAnime.playSpeed < 1.5f) {
                numEffects = (s32)(Rand_ZeroOne() * 30.0f) + 50;
                for (i = 0; i < numEffects; i++) {
                    EffectSsBubble_Spawn(globalCtx, &self->actor.world.pos, 60.0f, 100.0f, 50.0f, 0.15f);
                }
            }
        } else if (Animation_OnFrame(&self->skelAnime, sDoorAnimCloseFrames[self->animStyle])) {
            Audio_PlayActorSound2(&self->actor,
                                  (globalCtx->sceneNum == SCENE_HAKADAN || globalCtx->sceneNum == SCENE_HAKADANCH ||
                                   globalCtx->sceneNum == SCENE_HIDAN)
...
s32 EnDoor_OverrideLimbDraw(GlobalContext* globalCtx, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx) {
    ...
    if (limbIndex == 4) {
        temp_a2 = D_809FCEE4[self->dListIndex];
        transitionEntry = &globalCtx->transiActorCtx.list[(u16)self->actor.params >> 0xA];
        rot->z += self->actor.world.rot.y;
        if ((globalCtx->roomCtx.prevRoom.num >= 0) ||
            (transitionEntry->sides[0].room == transitionEntry->sides[1].room)) {
            phi_v0_2 = ((self->actor.shape.rot.y + self->skelAnime.jointTable[3].z) + rot->z) -
                       Math_Vec3f_Yaw(&globalCtx->view.eye, &self->actor.world.pos);
            *dList = (ABS(phi_v0_2) < 0x4000) ? temp_a2[0] : temp_a2[1];
        } else {
            phi_v0 = self->unk_192;
            if (transitionEntry->sides[0].room != self->actor.room) {
                phi_v0 ^= 1;
            }
...
void EnDoor_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (self->actor.objBankIndex == self->requiredObjBankIndex) {
        ...
        SkelAnime_DrawOpa(globalCtx, self->skelAnime.skeleton, self->skelAnime.jointTable, EnDoor_OverrideLimbDraw,
                          NULL, &self->actor);
        if (self->actor.world.rot.y != 0) {
            if (1) {}
            if (self->actor.world.rot.y > 0) {
        ...
        if (self->lockTimer != 0) {
            Actor_DrawDoorLock(globalCtx, self->lockTimer, 0);
        }
