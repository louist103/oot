...
void func_80ACDDE8(EnPart* self, GlobalContext* globalCtx) {
    f32 sign = 1.0f;

    self->action = 1;
    self->actor.world.rot.y = Rand_ZeroOne() * 20000.0f;

    switch (self->actor.params) {
        case 0:
            self->actor.velocity.y = 0.0f;
            self->actor.gravity = -0.3f - Rand_ZeroOne() * 0.5f;
            self->rotZSpeed = 0.3f;
            self->timer = 25;
            self->actor.speedXZ = (Rand_ZeroOne() - 0.5f) * 2.0f;
            break;
        case 13:
            self->timer = 400;
        case 12:
            self->actor.speedXZ = Rand_CenteredFloat(6.0f);
            self->actor.home.pos = self->actor.world.pos;
            self->timer += 60;
            self->actor.velocity.y = Rand_ZeroOne() * 5.0f + 4.0f;
            self->actor.gravity = -0.6f - Rand_ZeroOne() * 0.5f;
            self->rotZSpeed = 0.15f;
            break;
        case 14:
            EffectSsEnFire_SpawnVec3f(globalCtx, &self->actor, &self->actor.world.pos, 40, 0x8001, 0, -1);
        case 1:
        case 4:
        case 9:
        case 10:
            self->timer += (s16)(Rand_ZeroOne() * 17.0f) + 5;
        case 2:
            self->actor.velocity.y = Rand_ZeroOne() * 5.0f + 4.0f;
            self->actor.gravity = -0.6f - Rand_ZeroOne() * 0.5f;
            self->rotZSpeed = 0.15f;
        ...
        case 11:
            EffectSsEnFire_SpawnVec3f(globalCtx, &self->actor, &self->actor.world.pos, 40, 0x8001, 0, -1);
        case 3:
            self->actor.speedXZ = (Rand_ZeroOne() - 0.5f) * 3.0f;
            self->timer = (s16)(Rand_ZeroOne() * 17.0f) + 10;
            self->actor.velocity.y = Rand_ZeroOne() * 3.0f + 8.0f;
            self->actor.gravity = -0.6f - Rand_ZeroOne() * 0.3f;
            self->rotZSpeed = 0.15f;
            break;
        case 5:
        case 6:
        case 7:
        case 8:
            self->actor.world.rot.y = self->actor.parent->shape.rot.y;
            if (self->displayList == object_ik_DL_015380) {
                sign = -1.0f;
            }
            self->actor.velocity.y = 0.0f;
            self->actor.speedXZ = 6.0f * sign;
            self->actor.gravity = -1.2f;
            self->rotZSpeed = 0.15f * sign;
...
void func_80ACE13C(EnPart* self, GlobalContext* globalCtx) {
    s32 i;
    Vec3f pos;
    ...
    if ((self->actor.params == 12) || (self->actor.params == 13)) {
        Actor_UpdateBgCheckInfo(globalCtx, &self->actor, 5.0f, 15.0f, 0.0f, 0x1D);

        if ((self->actor.bgCheckFlags & 1) || (self->actor.world.pos.y <= self->actor.floorHeight)) {
            self->action = 4;
        ...
        if ((self->actor.params == 13) && (self->actor.parent != NULL) && (self->actor.parent->update == NULL)) {
        ...
        switch (self->actor.params) {
            ...
            case 10:
            case 14:
                EffectSsDeadDb_Spawn(globalCtx, &self->actor.world.pos, &zeroVec, &zeroVec,
                                     (s16)(self->actor.scale.y * 100.0f) * 40, 7, 255, 255, 255, 255, 0, 255, 0, 1, 9,
                                     true);
                break;
            case 3:
            case 11:
                EffectSsDeadDb_Spawn(globalCtx, &self->actor.world.pos, &zeroVec, &zeroVec,
                                     (s16)(self->actor.scale.y * 100.0f) * 40, 7, 255, 255, 255, 255, 0, 0, 255, 1, 9,
                                     true);
                break;
            case 4:
                for (i = 7; i >= 0; i--) {
                    pos.x = self->actor.world.pos.x + Rand_CenteredFloat(60.0f);
                    pos.y = self->actor.world.pos.y + self->actor.shape.yOffset * self->actor.scale.y +
                            Rand_CenteredFloat(50.0f);
                    pos.z = self->actor.world.pos.z + Rand_CenteredFloat(60.0f);
            ...
            case 8:
                for (i = 4; i >= 0; i--) {
                    pos.x = self->actor.world.pos.x + Rand_CenteredFloat(25.0f);
                    pos.y = self->actor.world.pos.y + Rand_CenteredFloat(40.0f);
                    pos.z = self->actor.world.pos.z + Rand_CenteredFloat(25.0f);
    ...
    self->timer--;
    self->rotZ += self->rotZSpeed;
}

void func_80ACE5B8(EnPart* self, GlobalContext* globalCtx) {
    self->action = 3;
}

void func_80ACE5C8(EnPart* self, GlobalContext* globalCtx) {
    ...
    if (self->timer == 0) {
        ...
        if (sqrt(self->actor.xyzDistToPlayerSq) <= 40.0f) {
            ...
            func_8002F71C(globalCtx, self->actor.parent, (650.0f - self->actor.parent->xzDistToPlayer) * 0.04f + 4.0f,
                          self->actor.parent->world.rot.y, 8.0f);
            player->invincibilityTimer = prevInvincibilityTimer;
            self->timer = 1;
        }

        func_80033480(globalCtx, &self->actor.world.pos, 0.0f, 1, 300, 150, 1);
        velocity.x = Rand_CenteredFloat(16.0f);
        EffectSsHahen_Spawn(globalCtx, &self->actor.world.pos, &velocity, &accel, 20,
                            (s32)((Rand_ZeroOne() * 5.0f + 12.0f) * 2), -1, 10, NULL);
        Audio_PlayActorSound2(&self->actor, NA_SE_EN_MONBLIN_GNDWAVE - SFX_FLAG);
    }
}

void func_80ACE7E8(EnPart* self, GlobalContext* globalCtx) {
    Vec3f zeroVec = { 0.0f, 0.0f, 0.0f };

    if ((self->actor.parent == NULL) || (self->actor.parent->update == NULL)) {
        EffectSsDeadDb_Spawn(globalCtx, &self->actor.world.pos, &zeroVec, &zeroVec,
                             (s16)(self->actor.scale.y * 100.0f) * 40, 7, 255, 255, 255, 255, 0, 255, 0, 1, 9, true);
        Actor_Kill(&self->actor);
        return;
    }

    if (self->timer == 0) {
        f32 diffsSum = Math_SmoothStepToF(&self->actor.world.pos.x, self->actor.home.pos.x, 1.0f, 5.0f, 0.0f);
        diffsSum += Math_SmoothStepToF(&self->actor.world.pos.y, self->actor.home.pos.y, 1.0f, 5.0f, 0.0f);
        diffsSum += Math_SmoothStepToF(&self->actor.world.pos.z, self->actor.home.pos.z, 1.0f, 5.0f, 0.0f);
        diffsSum += Math_SmoothStepToF(&self->rotZ, 0.0f, 1.0f, 0.25f, 0.0f);
        if (diffsSum == 0.0f) {
            self->actor.parent->home.rot.x--;
            self->timer--;
...
void EnPart_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if ((self->actor.params > 4 && self->actor.params < 9) || self->actor.params < 0) {
        Actor_UpdateBgCheckInfo(globalCtx, &self->actor, 5.0f, 15.0f, 0.0f, 5);
...
void EnPart_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisx->params > 0) {
        Matrix_RotateZ(self->rotZ, MTXMODE_APPLY);
    }
    ...
    } else if ((thisx->params == 9) && (self->displayList == object_tite_DL_002FF0)) {
    ...
    } else if ((thisx->params == 10) && (self->displayList == object_tite_DL_002FF0)) {
