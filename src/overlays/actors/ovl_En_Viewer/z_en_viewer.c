...
void EnViewer_GetCutsceneNextPos(EnViewer* self, GlobalContext* globalCtx);
void func_80B2C8AC(EnViewer* this2, GlobalContext* globalCtx);
void func_80B2CC1C(GlobalContext* globalCtx, EnViewer* self);
void func_80B2A570(EnViewer* self, GlobalContext* globalCtx);
void func_80B2A75C(EnViewer* self, GlobalContext* globalCtx);

// sAnimFuncs
void func_80B2A300(EnViewer* self, GlobalContext* globalCtx, void* skeletonHeaderSeg, AnimationHeader* animationSeg);
void func_80B2A448(EnViewer* self, GlobalContext* globalCtx, void* skeletonHeaderSeg, AnimationHeader* animationSeg);
void func_80B2A4D8(EnViewer* self, GlobalContext* globalCtx, void* skeletonHeaderSeg, AnimationHeader* animationSeg);

// sDrawFuncs
void func_80B2B4A8(EnViewer* self, GlobalContext* globalCtx);
void func_80B2B8FC(EnViewer* self, GlobalContext* globalCtx);
void func_80B2BA38(EnViewer* self, GlobalContext* globalCtx);
void func_80B2C130(EnViewer* self, GlobalContext* globalCtx);
...
void EnViewer_SetupAction(EnViewer* self, EnViewerActionFunc actionFunc) {
...
void EnViewer_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    EnViewer_SetupAction(self, func_80B2A570);
    D_80B2CEC0 = false;
    params = self->actor.params >> 8;
    self->unused = 0;
    self->unk_1E5 = 0;
    self->unk_1E6 = false;
    if (params == 3 || params == 5 || params == 7 || params == 8 || params == 9) {
        sGanonCape = (EnGanonMant*)Actor_SpawnAsChild(&globalCtx->actorCtx, &self->actor, globalCtx,
...
void func_80B2A300(EnViewer* self, GlobalContext* globalCtx, void* skeletonHeaderSeg, AnimationHeader* animationSeg) {
    s16 params = self->actor.params >> 8;

    if (params == 2 || params == 3 || params == 5 || params == 7 || params == 8 || params == 9) {
        SkelAnime_InitFlex(globalCtx, &self->skin.skelAnime, skeletonHeaderSeg, NULL, NULL, NULL, 0);
    } else {
        SkelAnime_Init(globalCtx, &self->skin.skelAnime, skeletonHeaderSeg, NULL, NULL, NULL, 0);
    }

    gSegments[6] = VIRTUAL_TO_PHYSICAL(globalCtx->objectCtx.status[self->animObjBankIndex].segment);
    if (params == 3 || params == 7 || params == 8 || params == 9) {
        Animation_PlayLoopSetSpeed(&self->skin.skelAnime, animationSeg, 1.0f);
    } else {
        Animation_PlayLoopSetSpeed(&self->skin.skelAnime, animationSeg, 3.0f);
    }
}
...
void func_80B2A4D8(EnViewer* self, GlobalContext* globalCtx, void* skeletonHeaderSeg, AnimationHeader* animationSeg) {
    ...
    func_800A663C(globalCtx, &self->skin, skeletonHeaderSeg, animationSeg);
    params = self->actor.params >> 8;
    if (!(params == 3 || params == 4 || params == 7 || params == 8 || params == 9)) {
        Animation_PlayLoopSetSpeed(&self->skin.skelAnime, animationSeg, 3.0f);
    } else {
        Animation_PlayOnceSetSpeed(&self->skin.skelAnime, animationSeg, 1.0f);
    }
}

void func_80B2A570(EnViewer* self, GlobalContext* globalCtx) {
    struct_80B2CEE8* unkStruct = &D_80B2CEE8[self->actor.params >> 8];
    s32 objIndex = Object_GetIndex(&globalCtx->objectCtx, unkStruct->objId1);

    ASSERT(objIndex >= 0, "bank_ID >= 0", "../z_en_viewer.c", 576);

    self->animObjBankIndex = Object_GetIndex(&globalCtx->objectCtx, unkStruct->objId2);
    ASSERT(self->animObjBankIndex >= 0, "self->anime_bank_ID >= 0", "../z_en_viewer.c", 579);

    if (!Object_IsLoaded(&globalCtx->objectCtx, objIndex) ||
        !Object_IsLoaded(&globalCtx->objectCtx, self->animObjBankIndex)) {
        self->actor.flags &= ~0x40;
        return;
    }

    self->unk_1E6 = true;
    self->actor.objBankIndex = objIndex;
    Actor_SetObjectDependency(globalCtx, &self->actor);
    Actor_SetScale(&self->actor, unkStruct->scale / 100.0f);
    ActorShape_Init(&self->actor.shape, unkStruct->unk_5 * 100,
                    sActorShadowDrawFunc[unkStruct->actorShadowDrawFuncIndex], unkStruct->unk_7);
    self->drawFuncIndex = unkStruct->drawFuncIndex;
    sAnimFuncs[self->drawFuncIndex](self, globalCtx, unkStruct->skel, unkStruct->unk_10);
    EnViewer_SetupAction(self, func_80B2A75C);
}
...
void func_80B2A75C(EnViewer* self, GlobalContext* globalCtx) {
    ...
    if (params == 2) {
        if (gSaveContext.sceneSetupIndex == 5) {
            ...
            if (csFrames == 792) {
                Audio_PlayActorSound2(&self->actor, NA_SE_VO_Z0_SURPRISE);
            } else if (csFrames == 845) {
                Audio_PlayActorSound2(&self->actor, NA_SE_VO_Z0_THROW);
            }
        }
    } else if (params == 7) {
        Actor_SetScale(&self->actor, 0.3f);
        self->actor.uncullZoneForward = 10000.0f;
        self->actor.uncullZoneScale = 10000.0f;
        self->actor.uncullZoneDownward = 10000.0f;
    } else if (params == 3) {
        ...
        if (gSaveContext.sceneSetupIndex == 5) {
            if (globalCtx->csCtx.frames == 1508) {
                Audio_PlayActorSound2(&self->actor, NA_SE_EN_FANTOM_ST_LAUGH);
            }
            if (globalCtx->csCtx.frames == 1545) {
                Actor_SpawnAsChild(&globalCtx->actorCtx, &self->actor, globalCtx, ACTOR_DEMO_6K, 32.0f, 101.0f, 1226.0f,
                                   0, 0, 0, 0xC);
            }
        }
        ...
        if (globalCtx->csCtx.frames == 960) {
            Audio_PlaySoundGeneral(NA_SE_EV_HORSE_GROAN, &self->actor.projectedPos, 4, &D_801333E0, &D_801333E0,
                                   &D_801333E8);
        }
    } else if (params == 6) {
        if (gSaveContext.sceneSetupIndex == 5 || gSaveContext.sceneSetupIndex == 10) {
            Audio_PlayActorSound2(&self->actor, NA_SE_EV_HORSE_RUN_LEVEL - SFX_FLAG);
        }
    } else if (params == 4) {
        curFrame = self->skin.skelAnime.curFrame;
        if (self->skin.skelAnime.animation == &gHorseGanonRearingAnim) {
            if (curFrame == 8) {
        ...
        } else if (self->skin.skelAnime.animation == &gHorseGanonIdleAnim) {
            if (curFrame == 25) {
                Audio_PlayActorSound2(&self->actor, NA_SE_EV_HORSE_SANDDUST);
            }
        } else if (self->skin.skelAnime.animation == &gHorseGanonGallopingAnim) {
            Audio_PlayActorSound2(&self->actor, NA_SE_EV_HORSE_RUN_LEVEL - SFX_FLAG);
        }
    }
    ...
    EnViewer_GetCutsceneNextPos(self, globalCtx);
    ...
    if (params == 3 || params == 4) {
        if (globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[1] != NULL) {
            if (globalCtx->csCtx.npcActions[1]->action == 2 && D_80B2CFCC == 0) {
                if (params == 3) {
                    if (self->skin.skelAnime.animation != &object_gndd_Anim_002928) {
                        Animation_PlayLoopSetSpeed(&self->skin.skelAnime, &object_gndd_Anim_002928, 1.0f);
                    }
                } else if (self->skin.skelAnime.animation != &gHorseGanonIdleAnim) {
                    Animation_PlayLoopSetSpeed(&self->skin.skelAnime, &gHorseGanonIdleAnim, 1.0f);
                }
            } else if (globalCtx->csCtx.npcActions[1]->action == 1) {
                D_80B2CFCC = 100;
                if (params == 3) {
                    if (self->skin.skelAnime.animation != &object_gndd_Anim_001D28) {
                        Animation_PlayLoopSetSpeed(&self->skin.skelAnime, &object_gndd_Anim_001D28, 1.0f);
                    }
                } else if (self->skin.skelAnime.animation != &gHorseGanonRearingAnim) {
                    Animation_PlayLoopSetSpeed(&self->skin.skelAnime, &gHorseGanonRearingAnim, 1.0f);
                }
            } else if (params == 3) {
                switch (self->unk_1E5) {
                    case 0:
                        if (globalCtx->csCtx.npcActions[1]->action == 4) {
                            Animation_MorphToPlayOnce(&self->skin.skelAnime, &object_gndd_Anim_000F54, -5.0f);
                            self->unk_1E5++;
                        }
                        break;
                    case 1:
                        if (animationEnded) {
                            Animation_MorphToLoop(&self->skin.skelAnime, &object_gndd_Anim_0014F4, -5.0f);
                            self->unk_1E5++;
                        }
                        break;
                    case 2:
                        if (globalCtx->csCtx.npcActions[1]->action == 5) {
                            Animation_MorphToPlayOnce(&self->skin.skelAnime, &object_gndd_Anim_0008A0, -5.0f);
                            self->unk_1E5++;
                        }
                        break;
                    case 3:
                        if (animationEnded) {
                            Animation_MorphToLoop(&self->skin.skelAnime, &object_gndd_Anim_000BC8, -5.0f);
                            self->unk_1E5++;
                        }
                        break;
                    case 4:
                        if (globalCtx->csCtx.npcActions[1]->action == 11) {
                            Animation_MorphToLoop(&self->skin.skelAnime, &object_gndd_Anim_0014F4, -20.0f);
                            self->unk_1E5++;
                        }
                        break;
                    case 5:
                        if (globalCtx->csCtx.npcActions[1]->action == 8) {
                            Animation_MorphToLoop(&self->skin.skelAnime, &object_gndd_Anim_002928, -15.0f);
                            self->unk_1E5++;
                        }
                        break;
                    case 6:
                        if (globalCtx->csCtx.npcActions[1]->action == 12) {
                            Audio_PlayActorSound2(&self->actor, NA_SE_EN_GANON_VOICE_DEMO);
                            Animation_PlayLoopSetSpeed(&self->skin.skelAnime, &object_gndd_Anim_0005B4, 3.0f);
                            self->unk_1E5++;
                        }
                        break;
                    case 7:
                ...
                Animation_PlayLoopSetSpeed(&self->skin.skelAnime, &gHorseGanonGallopingAnim, 3.0f);
            }
        }
    } else if (params == 1) {
        if (gSaveContext.sceneSetupIndex == 5) {
            if (globalCtx->csCtx.frames == 845) {
                Actor_SpawnAsChild(&globalCtx->actorCtx, &self->actor, globalCtx, ACTOR_ITEM_OCARINA, 4.0f, 81.0f,
                                   2600.0f, 0, 0, 0, 0);
            }
        } else {
        ...
        switch (self->unk_1E5) {
            case 0:
                if (globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[0] != NULL &&
                    ...
                    self->skin.skelAnime.animation != &object_opening_demo1_Anim_002574) {
                    Animation_PlayLoopSetSpeed(&self->skin.skelAnime, &object_opening_demo1_Anim_002574, 1.5f);
                    self->unk_1E5++;
                }
                break;
            case 1:
                if (globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[0] != NULL &&
                    ...
                    self->skin.skelAnime.animation != &object_opening_demo1_Anim_0029CC) {
                    Animation_PlayLoopSetSpeed(&self->skin.skelAnime, &object_opening_demo1_Anim_0029CC, 3.0f);
                    self->unk_1E5++;
                }
                break;
        }
        ...
        if (globalCtx->sceneNum == SCENE_SPOT00) { // Hyrule Field
                ...
                case 0:
                    if (globalCtx->csCtx.state != CS_STATE_IDLE) {
                        if (globalCtx->csCtx.npcActions[0] != NULL && globalCtx->csCtx.npcActions[0]->action == 6 &&
                            self->skin.skelAnime.animation != &object_opening_demo1_Anim_001410) {
                            Animation_PlayLoopSetSpeed(&self->skin.skelAnime, &object_opening_demo1_Anim_001410, 1.5f);
                            self->unk_1E5++;
                        }
                    }
                    break;
                case 1:
                    if (globalCtx->csCtx.state != CS_STATE_IDLE) {
                        if (globalCtx->csCtx.npcActions[0] != NULL && globalCtx->csCtx.npcActions[0]->action == 2 &&
                            self->skin.skelAnime.animation != &object_opening_demo1_Anim_000450) {
                            Animation_PlayLoopSetSpeed(&self->skin.skelAnime, &object_opening_demo1_Anim_000450, 3.0f);
                            self->unk_1E5++;
                        }
                    }
                    break;
            ...
            switch (self->unk_1E5) {
                case 0:
                    Animation_PlayLoopSetSpeed(&self->skin.skelAnime, &object_opening_demo1_Anim_00504C, 1.0f);
                    self->unk_1E5++;
                    break;
                case 1:
                    if (globalCtx->csCtx.npcActions[0]->action == 11) {
                        Animation_MorphToPlayOnce(&self->skin.skelAnime, &object_opening_demo1_Anim_00420C, -5.0f);
                        self->unk_1E5++;
                    }
                    break;
                case 2:
                    if (animationEnded) {
                        Animation_MorphToLoop(&self->skin.skelAnime, &object_opening_demo1_Anim_0048FC, -5.0f);
                        self->unk_1E5++;
                    }
                    break;
                case 3:
        ...
        switch (self->unk_1E5) {
            case 0:
                if (globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[1] != NULL &&
                    ...
                    Animation_MorphToPlayOnce(&self->skin.skelAnime, &object_gndd_Anim_004534, -5.0f);
                    self->unk_1E5++;
                }
                break;
            case 1:
                if (animationEnded) {
                    Animation_MorphToLoop(&self->skin.skelAnime, &object_gndd_Anim_0048B0, -5.0f);
                    self->unk_1E5++;
                }
                break;
        }
        ...
        switch (self->unk_1E5) {
            ...
            case 0:
                if (globalCtx->csCtx.state != CS_STATE_IDLE) {
                    if (globalCtx->csCtx.npcActions[1] != NULL && globalCtx->csCtx.npcActions[1]->action == 9) {
                        Animation_PlayLoopSetSpeed(&self->skin.skelAnime, &object_gndd_Anim_0050A8, 1.0f);
                        self->unk_1E5++;
                    }
                }
                break;
            case 1:
                if (globalCtx->csCtx.npcActions[1]->action == 10) {
                    Animation_MorphToPlayOnce(&self->skin.skelAnime, &object_gndd_Anim_003284, -10.0f);
                    self->unk_1E5++;
                }
                break;
            case 2:
                if (animationEnded) {
                    Animation_MorphToLoop(&self->skin.skelAnime, &object_gndd_Anim_003D84, -5.0f);
                    self->unk_1E5++;
                }
                break;
            case 3:
                if (globalCtx->csCtx.npcActions[1]->action == 4) {
                    Animation_MorphToPlayOnce(&self->skin.skelAnime, &object_gndd_Anim_003428, -5.0f);
                    self->unk_1E5++;
                }
                break;
        }
...
void func_80B2B4A8(EnViewer* self, GlobalContext* globalCtx) {
    ...
    if (params == 9) {
        SkelAnime_DrawFlexOpa(globalCtx, self->skin.skelAnime.skeleton, self->skin.skelAnime.jointTable,
                              self->skin.skelAnime.dListCount, NULL, func_80B2B364, self);
    } else if (params == 3) {
        SkelAnime_DrawFlexOpa(globalCtx, self->skin.skelAnime.skeleton, self->skin.skelAnime.jointTable,
                              self->skin.skelAnime.dListCount, func_80B2B2F4, func_80B2B468, self);
        func_80B2CC1C(globalCtx, self);
    } else if ((params == 3) || (params == 5) || (params == 7) || (params == 8)) {
        if ((globalCtx->csCtx.state != CS_STATE_IDLE) && (globalCtx->csCtx.npcActions[1] != NULL)) {
            SkelAnime_DrawFlexOpa(globalCtx, self->skin.skelAnime.skeleton, self->skin.skelAnime.jointTable,
                                  self->skin.skelAnime.dListCount, NULL, func_80B2B468, self);
            func_80B2CC1C(globalCtx, self);
        }
    } else {
        SkelAnime_DrawOpa(globalCtx, self->skin.skelAnime.skeleton, self->skin.skelAnime.jointTable, NULL, NULL, self);
    }
    CLOSE_DISPS(globalCtx->state.gfxCtx, "../z_en_viewer.c", 1511);
}

void func_80B2B8FC(EnViewer* self, GlobalContext* globalCtx) {
    func_800A6330(&self->actor, globalCtx, &self->skin, NULL, 1);
}
...
void func_80B2BA38(EnViewer* self, GlobalContext* globalCtx) {
    ...
    SkelAnime_DrawFlexOpa(globalCtx, self->skin.skelAnime.skeleton, self->skin.skelAnime.jointTable,
                          self->skin.skelAnime.dListCount, func_80B2B928, func_80B2B9A4, self);
...
void func_80B2C130(EnViewer* self, GlobalContext* globalCtx) {
    ...
    SkelAnime_DrawFlexOpa(globalCtx, self->skin.skelAnime.skeleton, self->skin.skelAnime.jointTable,
                          self->skin.skelAnime.dListCount, func_80B2C10C, NULL, self);
    ...
    if (self->unk_1E6) {
        ...
        if (params < 3) {
            if ((globalCtx->csCtx.state != CS_STATE_IDLE) && (globalCtx->csCtx.npcActions[0] != NULL)) {
                ...
                sDrawFuncs[self->drawFuncIndex](self, globalCtx);
            }
...
void EnViewer_GetCutsceneNextPos(EnViewer* self, GlobalContext* globalCtx) {
    ...
    if (params < 3) {
        if (globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[0] != NULL &&
            ...
            self->actor.world.pos.x = ((endPos.x - startPos.x) * interpolated) + startPos.x;
            self->actor.world.pos.y = ((endPos.y - startPos.y) * interpolated) + startPos.y;
            self->actor.world.pos.z = ((endPos.z - startPos.z) * interpolated) + startPos.z;
        }
    } else {
        if (globalCtx->csCtx.state != CS_STATE_IDLE && globalCtx->csCtx.npcActions[1] != NULL &&
            ...
            if (globalCtx->csCtx.npcActions[1]->action == 12) {
                ...
                Math_SmoothStepToS(&self->actor.world.rot.y, yaw, 10, 1000, 1);
                Math_SmoothStepToS(&self->actor.shape.rot.y, yaw, 10, 1000, 1);
            }

            if (params == 9) {
                self->actor.world.rot.x = globalCtx->csCtx.npcActions[1]->urot.x;
                self->actor.world.rot.y = globalCtx->csCtx.npcActions[1]->urot.y;
                self->actor.world.rot.z = globalCtx->csCtx.npcActions[1]->urot.z;
                self->actor.shape.rot.x = globalCtx->csCtx.npcActions[1]->urot.x;
                self->actor.shape.rot.y = globalCtx->csCtx.npcActions[1]->urot.y;
                self->actor.shape.rot.z = globalCtx->csCtx.npcActions[1]->urot.z;
            }
        }
        if (params == 5) {
            Audio_PlaySoundGeneral(NA_SE_EV_BURNING - SFX_FLAG, &D_801333D4, 4, &D_801333E0, &D_801333E0, &D_801333E8);
            func_80B2C8AC(self, globalCtx);
        }
    }
}

void func_80B2C768(EnViewer* self, GlobalContext* globalCtx, s16 arg2) {
    EnViewerUnkStruct* unkStruct;

    if ((arg2 % 2) == 0) {
        unkStruct = &self->unk_1E8[arg2];
...
void func_80B2C8AC(EnViewer* this2, GlobalContext* globalCtx) {
    ...
    for (i = 0; i < ARRAY_COUNT(self->unk_1E8); i++) {
        switch (self->unk_1E8[i].state) {
            case 0:
                func_80B2C768(self, globalCtx, i);
                self->unk_1E8[i].unk_24.z = (i >> 1) * 0.1f;
                self->unk_1E8[i].unk_24.x = 0.01f;
                self->unk_1E8[i].state++;
                break;
            case 1:
                Math_SmoothStepToF(&self->unk_1E8[i].unk_24.z, 1.0f, 1.0f, self->unk_1E8[i].unk_24.x,
                                   self->unk_1E8[i].unk_24.x);
                self->unk_1E8[i].unk_18.x =
                    self->unk_1E8[i].unk_0.x +
                    (self->unk_1E8[i].unk_C.x - self->unk_1E8[i].unk_0.x) * self->unk_1E8[i].unk_24.z;
                self->unk_1E8[i].unk_18.y =
                    self->unk_1E8[i].unk_0.y +
                    (self->unk_1E8[i].unk_C.y - self->unk_1E8[i].unk_0.y) * self->unk_1E8[i].unk_24.z;
                self->unk_1E8[i].unk_18.z =
                    self->unk_1E8[i].unk_0.z +
                    (self->unk_1E8[i].unk_C.z - self->unk_1E8[i].unk_0.z) * self->unk_1E8[i].unk_24.z;
                if (self->unk_1E8[i].unk_24.z >= 1.0f) {
                    self->unk_1E8[i].state++;
                }
                break;
            case 2:
                func_80B2C768(self, globalCtx, i);
                self->unk_1E8[i].unk_24.z = 0.0f;
                self->unk_1E8[i].unk_24.x = 0.01f;
                self->unk_1E8[i].state--;
                break;
        }
        ...
        func_80093D84(globalCtx->state.gfxCtx);
        Matrix_Translate(self->unk_1E8[i].unk_18.x, self->unk_1E8[i].unk_18.y, self->unk_1E8[i].unk_18.z, MTXMODE_NEW);
        Matrix_Scale(self->unk_1E8[i].unk_24.y, self->unk_1E8[i].unk_24.y, self->unk_1E8[i].unk_24.y, MTXMODE_APPLY);
