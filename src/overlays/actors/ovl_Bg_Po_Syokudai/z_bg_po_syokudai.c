...
void BgPoSyokudai_Init(Actor* thisx, GlobalContext* globalCtx) {
    BgPoSyokudai* self = THIS;
    s32 pad;

    Actor_ProcessInitChain(thisx, sInitChain);

    self->flameColor = (thisx->params >> 8) & 0xFF;
    thisx->params &= 0x3F;

    thisx->colChkInfo.mass = MASS_IMMOVABLE;

    self->lightNode = LightContext_InsertLight(globalCtx, &globalCtx->lightCtx, &self->lightInfo);
    Lights_PointGlowSetInfo(&self->lightInfo, thisx->world.pos.x, (s16)thisx->world.pos.y + 65, thisx->world.pos.z, 0,
                            0, 0, 0);

    Collider_InitCylinder(globalCtx, &self->collider);
    Collider_SetCylinder(globalCtx, &self->collider, thisx, &sCylinderInit);

    self->collider.dim.pos.x = thisx->world.pos.x;
    self->collider.dim.pos.y = thisx->world.pos.y;
    self->collider.dim.pos.z = thisx->world.pos.z;

    if (self->flameColor == POE_FLAME_PURPLE && Flags_GetSwitch(globalCtx, POE_TORCH_FLAG + POE_FLAME_GREEN) &&
        ...
        Actor_Spawn(&globalCtx->actorCtx, globalCtx, ACTOR_EN_PO_SISTERS, thisx->world.pos.x,
                    ...
                    (self->flameColor << 8) + thisx->params + 0x1000);
    ...
    self->flameTextureScroll = (s16)(Rand_ZeroOne() * 20.0f);
}
...
void BgPoSyokudai_Draw(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (Flags_GetSwitch(globalCtx, self->actor.params)) {
        Color_RGBA8* primColor = &sPrimColors[self->flameColor];
        Color_RGBA8* envColor = &sEnvColors[self->flameColor];
        ...
        Lights_PointSetColorAndRadius(&self->lightInfo, red, green, blue, 200);

        func_80093D84(globalCtx->state.gfxCtx);
        gSPSegment(POLY_XLU_DISP++, 0x08,
                   Gfx_TwoTexScroll(globalCtx->state.gfxCtx, 0, 0, 0, 32, 64, 1, 0,
                                    (self->flameTextureScroll * -20) & 0x1FF, 32, 128));
